...
const PREVIEW_HOST_ENV = "custom_PREVIEW_HOST";
...
* custom preview proxy so assets resolve when the page is loaded directly
* from the sandbox dev server (e.g. `localhost:8080`) instead of via
* `*.custom.app` / `*.customproject.com`.
*
* No-op unless `custom_PREVIEW_HOST` is set (host only, no scheme), e.g.
* `id-preview--<project-id>.custom.app`.
*/
function customAssetsProxyPlugin() {
	const previewHost = process.env[PREVIEW_HOST_ENV]?.trim();
	return {
		name: "custom:assets-proxy",
		apply: "serve",
		configureServer(server) {
			...
			const middleware = (req, res, next) => {
				...
				upstream.on("error", (err) => {
					server.config.logger.warn(`[@custom.dev/vite-tanstack-config] asset proxy ${url} → ${previewHost} failed: ${err.message}`);
					if (!res.headersSent) {
						res.statusCode = 502;
						res.end();
...
const BUNDLED_DEV_ENV_VAR = "custom_FEATURE_BUNDLED_DEV";
...
const VIRTUAL_PREFIX = "\0custom-bundled-dev-css-url:";
const VIRTUAL_SUFFIX = ".shim.js";
function bundledDevCssUrlShim() {
	return {
		name: "custom-bundled-dev-css-url-shim",
		apply: "serve",
		enforce: "pre",
...
//#endregion
//#region package.json
var name = "@custom.dev/vite-tanstack-config";
var version = "2.21.0";
//#endregion
//#region src/logging.ts
const PACKAGE_NAME = "@custom.dev/vite-tanstack-config";
...
const DOCUMENT_SET_DIR = ".custom";
const DOCUMENT_SET_FILE = "output.json";
const DOCUMENT_SET_PLUGIN_NAME = "custom:document-set";
...
const custom_SANDBOX_ENV_VAR = "custom_SANDBOX";
function isSandboxEnvironment() {
	return process.env[custom_SANDBOX_ENV_VAR] === "1" || !!process.env[SANDBOX_ENV_VAR];
}
...
const custom_NITRO_PRESETS = ["cloudflare-module", "custom-fetch-bundle"];
...
function pinPrerenderOptions(source, pins, label) {
	...
	for (const [key, value] of Object.entries(pins)) {
		if (key in result && result[key] !== value) logWarning(`Ignoring ${label}.${key} inside the custom build environment; forcing ${JSON.stringify(value)}.`);
		result[key] = value;
	}
	if ("outputPath" in result) {
		logWarning(`Ignoring ${label}.outputPath inside the custom build environment; prerendered routes are always written to <route>/index.html.`);
		delete result.outputPath;
	}
	return result;
}
const PRERENDER_REPORT_MARKER_OPEN = "[custom-prerender-report]";
const PRERENDER_REPORT_MARKER_CLOSE = "[/custom-prerender-report]";
...
function pinPrerenderForcustomBuild(userOptions) {
	...
	return {
		startOptions,
		skipReporter: {
			name: "custom:prerender-skip-report",
			apply: "build",
			enforce: "post",
			buildApp: {
				order: "post",
...
function customFetchBundlePreset() {
	return {
...
const SSR_CAPTURE_KEY = "__custom_TANSTACK_CAPTURE_SSR_ERROR__";
const HANDLER_RAN_KEY = "__custom_TANSTACK_HANDLER_RAN__";
...
const BUILD_DIAGNOSTICS_PLUGIN_NAME = "custom-build-error-diagnostics";
...
function stripRedundantNodejsCompatFlag(dirs) {
	let root = process.cwd();
	return {
		name: "custom-strip-redundant-nodejs-compat",
		apply: "build",
		configResolved(config) {
			root = config.root;
		},
...
const SHIM_MARKER = "// Generated by @custom.dev/vite-tanstack-config for the prerender preview server.";
...
function prerenderPreviewShim(state, dirs) {
	...
	return {
		name: "custom-prerender-preview-shim",
		apply: "build",
		config(userConfig) {
...
function prerenderPreviewShimCleanup(state) {
	return {
		name: "custom-prerender-preview-shim-cleanup",
		apply: "build",
		enforce: "post",
		buildApp: {
			order: "post",
...
function devSsrErrorLogger() {
	...
	const handlerRanFlag = Symbol("customHandlerRan");
	const capture = (error) => {
		lastCapture = {
			error,
			at: Date.now()
			...
			server.middlewares.use((req, res, next) => {
				...
				const handleErrorResponse = () => {
					...
					if (!res.headersSent) try {
						res.setHeader("x-custom-error-source", source);
					} catch {}
...
function defineConfig(configOrOptions = {}) {
	return async (env) => {
		...
		if (command === "build" && isSandbox) internalPlugins.push(customBuildErrorDiagnostics());
		...
		if (command === "build" && isSandbox) {
			const { startOptions, skipReporter, documentSet } = pinPrerenderForcustomBuild(tanstackStartOptions);
		...
		if (options.nitro !== false && command === "build") {
			...
			try {
				...
				if (explicitNitro || isSandbox) {
					const reason = explicitNitro ? `an explicit \`nitro\` option (${options.nitro === true ? "true" : "{ ... }"}) was set in vite.config.ts` : "running inside a custom sandbox";
					throw new Error(`[@custom.dev/vite-tanstack-config] ${reason}, but the \`nitro\` package isn't installed. Add \`nitro\` to your devDependencies (matches peerDependency >=3.0.260429-beta). Original import error: ${err instanceof Error ? err.message : String(err)}`);
				}
			}
			if (nitroMod) {
				...
				if (isSandbox) {
					for (const envVar of ["NITRO_PRESET", "SERVER_PRESET"]) if (process.env[envVar]) {
						logWarning(`Ignoring ${envVar} inside the custom build environment.`);
						delete process.env[envVar];
					}
					const requestedPreset = process.env.custom_NITRO_PRESET;
					if (requestedPreset && !custom_NITRO_PRESETS.includes(requestedPreset)) logWarning(`Ignoring unknown custom_NITRO_PRESET "${requestedPreset}"; building for cloudflare-module.`);
					if (requestedPreset === "custom-fetch-bundle") {
						if (!await nitroSupportsDefaultPreset()) throw new Error("[@custom.dev/vite-tanstack-config] custom_NITRO_PRESET=custom-fetch-bundle requires nitro >=3.0.260603-beta.");
						nitroOpts.preset = "";
						delete nitroOpts.cloudflare;
						const fetchBundle = customFetchBundlePreset();
		...
		if (command === "serve") internalPlugins.push(customAssetsProxyPlugin());
		let envDefine = {};
...
//#endregion
export { defineConfig, customAssetsProxyPlugin, stripRedundantNodejsCompatFlag };
