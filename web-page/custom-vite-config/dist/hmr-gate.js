...
const statusCorsOriginRe = /^(?:https:\/\/(?:[^/]+\.)?custom\.dev|https?:\/\/(?:(?:[^/:]+\.)?localhost|127\.0\.0\.1|\[::1\]))(?::\d+)?$/;
...
const DEV_SERVER_MODE_EVENT = "custom:dev-server-mode";
const BOOT_PROGRESS_EVENT = "custom:boot-progress";
const BOOT_PROGRESS_MIN_INTERVAL_MS = 500;
const UPDATE_PROGRESS_EVENT = "custom:update-progress";
...
const FULL_RELOAD_ENV_VAR = "custom_HMR_FULL_RELOAD";
function hmrGatePlugin(options = {}) {
	const fullReload = options.fullReload ?? process.env["custom_HMR_FULL_RELOAD"] !== "false";
	...
	function gateSendTarget(target) {
		...
		target.send = (...args) => {
			...
			if (type === "full-reload") {
				if (frame?.ifFallback === true || frame?.custom?.hmrGate === EDIT_FREE_RELOAD_MARKER || Date.now() < startupGraceUntil || initialBuildPending()) {
					passthroughReloads++;
	...
	return {
		...
		configResolved(config) {
			...
			const rawGeneration = process.env.custom_SPAWN_GENERATION;
