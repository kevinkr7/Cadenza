...
const DEFAULT_ENDPOINT = "/__custom/error-collector";
...
function browserClientScript(endpoint) {
	return `
(() => {
  if (globalThis.__customErrorCollectorInstalled) return;
  globalThis.__customErrorCollectorInstalled = true;
...
const RESTART_ANNOUNCE_PATH = "/__custom/restart-announce";
const RESTART_EVENT = "custom:dev-server-restarting";
const PREVIEW_EXECUTE_PATH = "/_sandbox/preview/execute";
const PREVIEW_VIEWERS_PATH = "/_sandbox/preview/viewers";
const PREVIEW_EXECUTE_EVENT = "custom:preview-execute";
const PREVIEW_EXECUTE_RESULT_EVENT = "custom:preview-execute-result";
const PREVIEW_EXECUTE_CLIENT_MODULE_ID = "virtual:custom-preview-execute-client";
const RESOLVED_PREVIEW_EXECUTE_CLIENT_MODULE_ID = "\0virtual:custom-preview-execute-client";
...
if (import.meta.hot) {
  ...
  const viewerID =
    globalThis.__customPreviewExecuteViewerID ||
    (globalThis.__customPreviewExecuteViewerID =
      globalThis.crypto?.randomUUID?.() ?? Math.random().toString(36).slice(2));
  const handled =
    globalThis.__customPreviewExecuteHandled || (globalThis.__customPreviewExecuteHandled = new Set());
